...
#' Generate recovery time (impulse response anlaysis metric) from beta matrix.
#'
#' @param beta.matrix beta matrix
...
impulse.response.boot <- function(replication = 200, var.number, lag.order, bootstrapped.beta, steps)
{
  simulation <- NULL
...
iRAM.boot <- function(model.fit,
  ...
  recovery.time.ci.lower <- matrix(rep(0, var.number*var.number), nrow = 1)
                 ...
                 # plot.histogram = FALSE)
{
  ...
  else
  {
    ...
    # print(beta.matrix)
    compute.recovery.time.from.beta(beta.matrix$est,  var.number, lag.order, threshold , replication, steps)
  }
}
